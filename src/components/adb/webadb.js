...
(function (root, factory) {
  ...
  // 'use strict'; // not using because of warning in react
  ...
  Adb.open = function (transport) {
    if (transport === "WebUSB") return Adb.WebUSB.Transport.open();
  ...
  Adb.WebUSB.Transport = function (device) {
    ...
    if (Adb.Opt.debug) console.log(this);
  };
  ...
  Adb.WebUSB.Transport.open = function () {
    ...
    return navigator.usb
      ...
      .then((device) =>
        device.open().then(() => new Adb.WebUSB.Transport(device))
      );
  };
  ...
  Adb.WebUSB.Transport.prototype.send = function (ep, data) {
    if (Adb.Opt.dump) hexdump(new DataView(data), "" + ep + "==> ");

    return this.device.transferOut(ep, data);
  };

  Adb.WebUSB.Transport.prototype.receive = function (ep, len) {
    return this.device.transferIn(ep, len).then((response) => {
      if (Adb.Opt.dump) hexdump(response.data, "<==" + ep + " ");
    ...
    for (let i in this.device.configurations) {
      ...
      for (let j in conf.interfaces) {
        ...
        for (let k in intf.alternates) {
          ...
          if (
            filter.classCode === alt.interfaceClass &&
  ...
  Adb.WebUSB.Transport.prototype.getDevice = function (filter) {
    ...
    return this.device
      .selectConfiguration(match.conf.configurationValue)
      .then(() => this.device.claimInterface(match.intf.interfaceNumber))
      .then(() =>
        this.device.selectAlternateInterface(
          match.intf.interfaceNumber,
          match.alt.alternateSetting
        )
      )
      .then(() => match);
  };

  Adb.WebUSB.Transport.prototype.connectAdb = function (
    banner,
    auth_user_notify = null
  ) {
      ...
      .then((adb) =>
        m
          ...
          .then((response) => {
            ...
            if (
              response.arg0 !== VERSION &&
              response.arg0 !== VERSION_NO_CHECKSUM
            )
              throw new Error(
                "Version mismatch: " +
                  response.arg0 +
            ...
            if (Adb.Opt.debug)
              console.log(
                "Connected with '" + banner + "', max_payload: " + response.arg1
              );
  ...
  Adb.WebUSB.Transport.prototype.connectFastboot = function () {
    return this.getDevice({ classCode: 255, subclassCode: 66, protocolCode: 3 })
      ...
      .then((fastboot) =>
        fastboot.send("getvar:max-download-size").then(() =>
          fastboot.receive().then((response) => {
            let cmd = decode_cmd(response.getUint32(0, true));
            if (cmd === "FAIL") throw new Error("Unable to open Fastboot");
  ...
  Adb.Message = function (cmd, arg0, arg1, data = null) {
    if (cmd.length !== 4) throw new Error("Invalid command: '" + cmd + "'");
  ...
  Adb.Message.checksum = function (data_view) {
    ...
    for (let i = 0; i < data_view.byteLength; i++) sum += data_view.getUint8(i);
  ...
  Adb.Message.send = function (device, message) {
    ...
    if (Adb.Opt.debug) console.log(message);
    ...
    if (len > 0) seq.then(() => device.send(data));
    return seq;
  };

  Adb.Message.receive = function (device) {
    return device
      .receive(24) //Adb.Opt.use_checksum ? 24 : 20)
      .then((response) => {
        ...
        if (Adb.use_checksum && response.byteLength > 20) {
          ...
          if ((cmd ^ magic) !== -1) throw new Error("magic mismatch");
        }
        ...
        return device.receive(len).then((data) => {
  ...
  Adb.Message.prototype.send_receive = function (device) {
    return this.send(device).then(() => Adb.Message.receive(device));
  };
  ...
  Adb.SyncFrame.send = function (stream, frame) {
    ...
    if (Adb.Opt.debug) console.log(frame);
  ...
  Adb.SyncFrame.receive = function (stream) {
    return stream.receive().then((response) => {
      if (response.cmd === "WRTE") {
        ...
        if (Adb.Opt.debug) console.log(response);
        if (Adb.Opt.dump) hexdump(response.data, "WRTE: ");
  ...
  Adb.SyncFrame.prototype.send_receive = function (stream) {
    return Adb.SyncFrame.send(stream, this).then(() =>
      Adb.SyncFrame.receive(stream)
    );
  };
  ...
  Adb.Stream.open = function (device, service) {
    ...
    return m.send_receive(device).then(function do_response(response) {
      if (response.arg1 !== local_id)
        return Adb.Message.receive(device).then(do_response);

      if (response.cmd !== "OKAY") throw new Error("Open failed");
  ...
  Adb.Stream.prototype.send_receive = function (cmd, data = null) {
    return this.send(cmd, data).then(() => this.receive());
  };

  Adb.Stream.prototype.abort = function () {
    if (Adb.Opt.debug) console.log("aborting...");

    let self = this;
    return new Promise(function (resolve, reject) {
      self.cancel = function () {
        if (Adb.Opt.debug) console.log("aborted");
        self.cancel = null;
        resolve();
      };
    });
    ...
    return frame
      ...
      .then((response) => {
        return this.receive().then((response) =>
          this.send("OKAY").then(() => response.data)
        );
      })
      .then((response) => {
        ...
        if (id !== "STAT") throw new Error("STAT failed on " + filename);
  ...
  Adb.Stream.prototype.pull = function (filename) {
    ...
    return frame
      ...
      .catch((err) => {
        return this.send("OKAY").then(() => {
          throw err;
        });
      })
      .then((response) => {
        return this.send("OKAY").then(() => response);
      })
      .then((response) => {
        ...
        if (response.data.byteLength > 64 * 1024) {
          ...
          let zero = response.data.getUint32(
  ...
  Adb.Stream.prototype.push_start = function (filename, mode) {
    ...
    let frame = new Adb.SyncFrame(
      "SEND",
      filename.length + 1 + mode_str.length
    );
    return frame
      .send_receive(this)
  ...
  Adb.Stream.prototype.push = function (
    file,
    filename,
    mode,
    on_progress = null
      ...
      this.push_start(filename, mode).then(() => {
        ...
        while (rem > 0) {
          ...
          seq = seq.then(() => {
            ...
            if (on_progress !== null) on_progress(count, file.size);
  ...
  function check_cmd(cmd, err_msg) {
    return function (response) {
      ...
      if (response.cmd !== cmd) throw new Error(err_msg);
      return response;
    };
  }
  ...
  function paddit(text, width, padding) {
    ...
    for (let i = 0; i < padlen; i++) padded += padding;
  ...
  function hexdump(view, prefix = "") {
    ...
    for (let i = 0; i < view.byteLength; i += 16) {
      ...
      for (j = 0; j < max; j++) row += " " + toHex8(view.getUint8(i + j));
      for (; j < 16; j++) row += "   ";
  ...
  function get_ep_num(endpoints, dir, type = "bulk") {
    ...
    if (Adb.Opt.debug) console.log(endpoints);
  ...
  function do_auth(
    ...
    if (key_idx < keys.length) {
      ...
      if (Adb.Opt.debug) console.log("signing with key " + slot + "...");
      if (Adb.Opt.dump) {
        seq = seq
          .then(() => privkey_dump(key))
          .then(() => pubkey_dump(key))
    ...
    if (Adb.Opt.reuse_key !== false) {
      ...
      if (key_idx >= 0 && key_idx < keys.length) {
        if (Adb.Opt.debug) console.log("reusing key " + key_idx + "...");
    ...
    if (seq === null) {
      if (Adb.Opt.debug)
        console.log(
          "generating key " + key_idx + " (" + Adb.Opt.key_size + " bits)..."
        );
    ...
    return seq.then((key) => {
      return crypto.subtle
        ...
        .then(() => {
          if (Adb.Opt.debug) console.log("waiting for user confirmation...");
          if (auth_user_notify !== null) auth_user_notify(key.publicKey);
          return Adb.Message.receive(adb);
        })
        .then((response) => {
          // return response;
          if (response.cmd !== "CNXN") return response;
          if (!dirty) return response;

          keys.push(key);
          return db.then((db) => store_key(db, key)).then(() => response);
        });
    });
  }
  ...
  function init_db() {
    ...
    req.onupgradeneeded = function (event) {
      ...
      if (db.objectStoreNames.contains("keys")) {
        if (Adb.Opt.debug) console.log("DB: deleting old keys...");
  ...
  function load_keys(db) {
    ...
    return promisify(transaction, "oncomplete").then(function (result) {
      if (Adb.Opt.debug) console.log("DB: loaded " + keys.length + " keys");
      return keys;
    });
  }
  ...
  function store_key(db, key) {
    ...
    return promisify(request).then(function (result) {
      if (Adb.Opt.debug) console.log("DB: stored key " + (result - 1));
      return result;
    });
  }
  ...
  function clear_keys(db) {
    ...
    return promisify(request).then(function (result) {
      if (Adb.Opt.debug) console.log("DB: removed all the keys");
      return result;
    });
  }
